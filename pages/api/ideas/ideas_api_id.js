...
export default async function handler(req, res) {
  ...
  try {
    ...
    const [itemRes, commentsRes, votesRes, optionsRes] = await Promise.all([
      fetch(`${SUPABASE_URL}/rest/v1/feature_requests?id=eq.${id}&select=*`, { headers }),
      fetch(`${SUPABASE_URL}/rest/v1/feature_comments?request_id=eq.${id}&select=comment,created_at,user_profiles(display_name)&order=created_at.asc`, { headers }),
      fetch(`${SUPABASE_URL}/rest/v1/feature_votes?request_id=eq.${id}&select=user_id`, { headers }),
      fetch(`${SUPABASE_URL}/rest/v1/feature_request_options?request_id=eq.${id}&select=id,option_text,display_order&order=display_order.asc`, { headers })
    ]);
    ...
    if (Array.isArray(optionsData) && optionsData.length > 0) {
      ...
      const countsRes = await fetch(`${SUPABASE_URL}/rest/v1/rpc/get_poll_vote_counts`, { method: 'POST', headers });
