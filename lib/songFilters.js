 ...
 * @param filters - {
 *   songbookIds: [],        // multi-select; [] means "any songbook"
 *   songbookMode: 'any',    // 'any' = song needs to be in at least one selected songbook (OR)
 *                           // 'all' = song needs to be in every selected songbook (AND)
 *   sections: [],           // multi-select; values are real section_id's
 *   sectionMode: 'any',     // same any/all meaning as songbookMode, for sections
 *   excludeSections: [],    // if song has ANY of these sections, it's filtered out - always OR, same convention as excludeTagIds
 ...
 *   personalTagValues: [],  // multi-select; [] means "no restriction"
 *   personalTagMode: 'any'  // same any/all meaning, for personalTagValues
 * }
 *
 ...
 * real ids.
 *
 * songbookMode/sectionMode/personalTagMode are all new, optional parameters
 * that default to 'any' - the exact behavior every page already had before
 * these existed. A page that doesn't pass them (room, admin tags) behaves
 * identically to before; only a page that explicitly opts in (songs.js) gets
 * the 'all' option.
 */
export function songMatchesFilters(entries, songTagIds, personalTags, filters) {
  const {
    songbookIds = [], songbookMode = 'any',
    sections = [], sectionMode = 'any', excludeSections = [],
    includeTagIds = [], includeMode = 'any', excludeTagIds = [],
    personalTagValues = [], personalTagMode = 'any'
  } = filters;

  if (songbookIds.length > 0) {
    const matches = songbookMode === 'all'
      ? songbookIds.every(id => entries.some(e => e.songbook_id === id))
      : entries.some(e => songbookIds.includes(e.songbook_id));
    if (!matches) return false;
  }

  // Exclude-by-section always wins over include, same convention as tags:
  // relevant is scoped to the selected songbook(s) if any, same as include,
  // so excluding "A: Graces" in one songbook doesn't also exclude a song
  // that's only in a different songbook's unrelated "A: Graces".
  if (excludeSections.length > 0) {
    const relevantForExclude = songbookIds.length > 0
      ? entries.filter(e => songbookIds.includes(e.songbook_id))
      : entries;
    const hasExcluded = relevantForExclude.some(e => excludeSections.includes(e.section_id));
  ...
  if (sections.length > 0) {
    const relevant = songbookIds.length > 0
      ? entries.filter(e => songbookIds.includes(e.songbook_id))
      : entries;
    const matches = sectionMode === 'all'
      ? sections.every(id => relevant.some(e => e.section_id === id))
      : relevant.some(e => sections.includes(e.section_id));
    if (!matches) return false;
  }
  ...
  if (personalTagValues.length > 0) {
    const matches = personalTagMode === 'all'
      ? personalTagValues.every(t => (personalTags || []).includes(t))
      : personalTagValues.some(t => (personalTags || []).includes(t));
    if (!matches) return false;
  }
...
export function emptyFilterState() {
  return { songbookIds: [], songbookMode: 'any', sections: [], sectionMode: 'any', excludeSections: [], includeTagIds: [], includeMode: 'any', excludeTagIds: [], personalTagValues: [], personalTagMode: 'any' };
}
